...
local function CreateColumnHeaders(content)
    ...
    local rankButton = CreateColumnHeader(content, "Rank", colWidths.rank, guildButton, 0, 0, "rank")
    local zoneButton = CreateColumnHeader(content, "Zone", colWidths.zone, rankButton, 0, 0, "zone")
...
local function CreateEntryRow(content, entry, yOffset, colWidths, isAlternate)
    ...
    local rankCell = CreateRankCell(rowContainer, guildCell, entry.rank, colWidths.rank)
    local zoneCell = CreateZoneCell(rowContainer, rankCell, entry.zone, colWidths.zone)
