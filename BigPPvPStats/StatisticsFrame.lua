...
local function getChartOrder()
    if not BPP_DB or not BPP_DB.StatisticsChartOrder then
        return copyArray(DEFAULT_CHART_ORDER)
    end
    local present = {}
    for _, id in ipairs(BPP_DB.StatisticsChartOrder) do
        present[id] = true
    end
    local order = copyArray(BPP_DB.StatisticsChartOrder)
...
local function saveChartOrder(order)
    BPP_DB.StatisticsChartOrder = order
end
...
local function createBar(container, entry, index, maxValue, total, titleType, disableClicks, extraData)
    ...
    if isClickable then
        local highlightTexture = BPP_CreateGoldHighlight(barButton, UI.BAR.HEIGHT)
    end
    ...
    if isClickable then
        barButton:SetScript("OnClick", function()
            ...
            C_Timer.After(0.05, function()
                if titleType == "class" or titleType == "unknownLevelClass" then
                    BPP_SetKillListSearch("", nil, entry.key, nil, nil, nil, true)
                elseif titleType == "deathsClass" then
                    BPP_SetKillListSearch("", nil, entry.key, nil, nil, nil, true)
                    BPP_SetKillListMinDeaths(1)
                elseif titleType == "zone" then
                    BPP_SetKillListSearch("", nil, nil, nil, nil, entry.key, true)
                elseif titleType == "level" then
                    if entry.key == "??" then
                        BPP_SetKillListLevelRange(-1, -1, true)
                    else
                        local level = tonumber(entry.key)
                        if level then
                            BPP_SetKillListLevelRange(level, level, true)
                        end
                    end
                elseif titleType == raceColors then
                    BPP_SetKillListSearch("", nil, nil, entry.key, nil, nil, true)
                elseif titleType == genderColors then
                    BPP_SetKillListSearch("", nil, nil, nil, entry.key, nil, true)
                end
...
function BPP_GetWinPercentageColor(pct)
    if pct == 100 then
...
local function createKDByClassBarChart(parent, x, y, width, kdData, rawData)
    ...
    for i, entry in ipairs(entries) do
        ...
        barButton:SetPoint("TOPLEFT", 0, barY)
        BPP_CreateGoldHighlight(barButton, UI.BAR.HEIGHT)
        ...
        barButton:SetScript("OnEnter", function(self)
            ...
            if totalEncounters > 0 then
                ...
                local wc = BPP_GetWinPercentageColor(winPct)
...
local function BPP_ExtractNameAndLevelKey(nameWithLevel)
...
local function BPP_GetMonthBucketFromTimestamp(timestamp)
...
local function BPP_FormatMonthBucketLabel(bucket)
...
function BPP_CalculateMonthlyClassPercentageStatistics(charactersToProcess)
    local monthlyTotals = {}
    ...
    if not BPP_DB.PlayerKillCounts.Characters then
        ...
        if characterData.Kills then
            for nameWithLevel, killData in pairs(characterData.Kills) do
                if killData.killLocations and #killData.killLocations > 0 then
                    local enemyName = BPP_ExtractNameAndLevelKey(nameWithLevel)
                    local infoKey = BPP_NormalizePlayerName(enemyName)
                    local info = BPP_DB.PlayerInfoCache[infoKey]
                    local className = info and info.class or nil

                    for _, location in ipairs(killData.killLocations) do
                        local bucket = BPP_GetMonthBucketFromTimestamp(location.timestamp)
                        if bucket then
...
local function createMonthlyClassPercentageChart(parent, x, y, width, disableClicks, monthKeys, monthlyClassPercentages, monthlyTotals)
    ...
    for i, monthBucket in ipairs(monthKeys) do
        ...
        if i == 1 or i == monthCount or (i - 1) % labelStep == 0 then
            ...
            xLabel:SetText(BPP_FormatMonthBucketLabel(monthBucket))
        end
    end
            ...
            if value and value > 0 then
                ...
                if not disableClicks then
                    ...
                    pointButton:SetScript("OnEnter", function(self)
                        ...
                        GameTooltip:AddLine(BPP_FormatMonthBucketLabel(monthBucket), 1, 1, 1)
                        ...
                        if previousValue ~= nil then
                            ...
                            if delta > 0 then
                                GameTooltip:AddLine(string.format("Delta vs %s: %s%% increase", BPP_FormatMonthBucketLabel(previousMonthBucket), deltaText), 0.3, 1, 0.3, true)
                            elseif delta < 0 then
                                GameTooltip:AddLine(string.format("Delta vs %s: %s%% decrease", BPP_FormatMonthBucketLabel(previousMonthBucket), deltaText), 1, 0.3, 0.3, true)
                            else
                                GameTooltip:AddLine(string.format("Delta vs %s: no change", BPP_FormatMonthBucketLabel(previousMonthBucket)), 0.85, 0.85, 0.85, true)
                            end
                        else
    ...
    for classIndex, className in ipairs(ALL_CLASSES) do
        ...
        BPP_CreateGoldHighlight(legendButton, 12)
...
local function createGuildTableRow(content, entry, index, firstRowSpacing)
    ...
    local highlightTexture = BPP_CreateGoldHighlight(rowButton, 16)
    ...
    rowButton:SetScript("OnClick", function()
        ...
        if BPP_KillsListFrame then
            ...
            BPP_KillsListFrame:SetFrameLevel(statisticsFrame:GetFrameLevel() + 10)
        end

        BPP_SetKillListSearch(guildName, nil, nil, nil, nil, nil, true)
    end)
end
...
function BPP_CalculateGuildKills(charactersToProcess)
    local guildKills = {}
    local scope = charactersToProcess or BPP_GetCharactersToProcessForStatistics()
    ...
    for _, characterData in pairs(scope) do
        if characterData.Kills then
            for nameWithLevel, killData in pairs(characterData.Kills) do
                ...
                local infoKey = BPP_NormalizePlayerName(playerNameWithoutLevel)
                if BPP_DB.PlayerInfoCache[infoKey] then
                    local guild = BPP_DB.PlayerInfoCache[infoKey].guild
                    if guild ~= "" then
...
local function BPP_SummaryStats_CreateState(charactersToProcess)
    local BPP_DB = BPP_DB
---@diagnostic disable-next-line: undefined-global
    local BPP_CalculateTimePeriodBoundaries = BPP_CalculateTimePeriodBoundaries
    ...
    state.timeBoundaries = BPP_CalculateTimePeriodBoundaries()

    return state
end

local function BPP_SummaryStats_ProcessCharacterHeader(state, characterKey, characterData)
    local BPP_GetCharacterKey = BPP_GetCharacterKey
    local BPP_DB = BPP_DB

    if characterKey == BPP_GetCharacterKey() then
...
local function BPP_SummaryStats_ProcessKillEntryBase(state, nameWithLevel, killData)
    ...
    playerName = BPP_NormalizePlayerName(playerName)
...
local function BPP_SummaryStats_ProcessKillLocation(state, location, levelNum)
    local tonumber = tonumber
    local date = date
...
local function BPP_SummaryStats_ProcessKillEntryFallback(state, killData, kills, levelNum)
...
local function BPP_SummaryStats_FinalizeKillDerivedFields(state)
    local pairs = pairs
    local ipairs = ipairs
...
local function BPP_SummaryStats_FinalizeNemesisAndDeaths(state, characterScope)
    local pairs = pairs
    local ipairs = ipairs
    local math_huge = math.huge
    local BPP_GetDeathDataFromAllCharacters = BPP_GetDeathDataFromAllCharacters

    local nemesisName = "None"
    local nemesisScore = 0
    local nemesisAssists = 0
    local deathDataByPlayer = BPP_GetDeathDataFromAllCharacters(characterScope)

    local assistsByPlayer = {}
    local deathsByPlayer = {}

    for killerName, deathData in pairs(deathDataByPlayer) do
        local normalizedKiller = BPP_NormalizePlayerName(killerName)
                ...
                if location.assisters then
                    for _, assister in ipairs(location.assisters) do
                        if assister and assister.name then
                            local normalizedAssister = BPP_NormalizePlayerName(assister.name)
...
local function BPP_SummaryStats_BuildResult(state)
    return {
...
function BPP_CalculateDeathsByClass(characterScope)
    local deathsByClass = {}
    ...
    for killerName, deathData in pairs(deathDataByPlayer) do
        local infoKey = BPP_NormalizePlayerName(killerName)
        local info = BPP_DB.PlayerInfoCache[infoKey]
...
local function BPP_CalculateKDByClass(killsByClass, deathsByClass)
    local kdData = {}
    local rawData = {}
...
local function addSummaryStatLine(container, label, value, yPosition, tooltipText, isKillStreak, isLocalPlayer, valueColor)
    ...
    if tooltipText then
        ...
        if label == "Most killed player:" and isLocalPlayer then
            ...
            BPP_CreateGoldHighlight(button, 20)
            ...
            button:SetScript("OnMouseUp", function()
                if value ~= "None (0)" then
                    ...
                    BPP_CreateKillsListFrame()
                    C_Timer.After(0.05, function()
                        BPP_SetKillListSearch(playerName, nil, nil, nil, nil, nil, true)
                        BPP_FrameManager:BringToFront("KillsList")
                    end)
                end
            end)
...
function BPP_CalculateSummaryStatistics(charactersToProcess)
    local state = BPP_SummaryStats_CreateState(charactersToProcess)

    for characterKey, characterData in pairs(charactersToProcess) do
        BPP_SummaryStats_ProcessCharacterHeader(state, characterKey, characterData)

        if characterData.Kills then
            for nameWithLevel, killData in pairs(characterData.Kills) do
                local kills, _, levelNum = BPP_SummaryStats_ProcessKillEntryBase(state, nameWithLevel, killData)

                if killData.killLocations and #killData.killLocations > 0 then
                    for _, location in ipairs(killData.killLocations) do
                        BPP_SummaryStats_ProcessKillLocation(state, location, levelNum)
                    end
                else
                    BPP_SummaryStats_ProcessKillEntryFallback(state, killData, kills, levelNum)
                end
            end
        end
    end

    BPP_SummaryStats_FinalizeKillDerivedFields(state)
    BPP_SummaryStats_FinalizeNemesisAndDeaths(state, charactersToProcess)
    return BPP_SummaryStats_BuildResult(state)
end

function BPP_CreateIncrementalSummaryStatisticsTask(charactersToProcess, maxKillLocationsPerFrame, onComplete)
    ...
    return function()
        ...
        while processed < sliceBudget do
            if characterIndex > #characterKeys then
                ...
                BPP_SummaryStats_FinalizeNemesisAndDeaths(state, charactersToProcess)
                finished = true
                onComplete(BPP_SummaryStats_BuildResult(state))
                return true
            end
                        ...
                        if not killData then
                            ...
                            if currentKillData.killLocations and #currentKillData.killLocations > 0 then
                                ...
                                if locationIndex > #locations then
                                    ...
                                    if location then
                                        BPP_SummaryStats_ProcessKillLocation(state, location, currentKillLevelNum)
                                    end
                                    locationIndex = locationIndex + 1
                                    processed = processed + 1
                                end
                            else
                                BPP_SummaryStats_ProcessKillEntryFallback(state, currentKillData, currentKillKills, currentKillLevelNum)
                                advanceToNextKill()
                                processed = processed + 1
                            end
                        end
...
local function BPP_PopulateSummaryStatsContainer(container, stats, isLocalPlayer, extraData, playerName)
    ...
    local kdRatio = BPP_FormatKDRatio(stats.totalKills, stats.totalDeaths, stats.kdRatio)
    ...
    if totalEncounters > 0 then
        ...
        local winColor = BPP_GetWinPercentageColor(winPct)
    ...
    if stats.mostKilledPlayer and (stats.mostKilledCount or 0) > 0 and stats.mostKilledPlayer ~= "None" then
        ...
        if isLocalPlayer then
            ...
            if not mkRace or not mkGender or not mkClass then
                local mkInfo, _ = BPP_GetPlayerInfo(stats.mostKilledPlayer)
    ...
    if stats.nemesisName and stats.nemesisName ~= "None" and (stats.nemesisScore or 0) > 0 then
        ...
        if isLocalPlayer then
            ...
            if not nemesisRace or not nemesisGender or not nemesisClass then
                local nemesisInfo, _ = BPP_GetPlayerInfo(stats.nemesisName)
    ...
    if extraData and extraData.achievementsUnlocked and extraData.totalAchievements then
        ...
        if isLocalPlayer then
            ...
            BPP_CreateGoldHighlight(achievementButton, 20)
            ...
            achievementButton:SetScript("OnClick", function() BPP_ToggleAchievementFrame() end)
        else
...
local function createSummaryStatsForExternalPlayer(parent, x, y, width, height, stats, playerName, extraData)
    ...
    BPP_PopulateSummaryStatsContainer(container, stats, false, extraData, playerName)
    return container
end
...
local function createSummaryStats(parent, x, y, width, height)
    ...
    if PVPSC.AchievementSystem and PVPSC.AchievementSystem.achievements then
        ...
        if BPP_DB.CharacterAchievements and BPP_DB.CharacterAchievements[currentCharacterKey] then
            for _, achievementData in pairs(BPP_DB.CharacterAchievements[currentCharacterKey]) do
        ...
        extraData.achievementPoints = BPP_DB.CharacterAchievementPoints[currentCharacterKey] or 0
        extraData.totalPossiblePoints = PVPSC.AchievementSystem:GetTotalPossiblePoints()
    end

    BPP_PopulateSummaryStatsContainer(container, stats, true, extraData)
    return container
end

function BPP_CalculateBarChartStatistics(charactersToProcess)
    local db = BPP_DB
    local playerInfoCache = db.PlayerInfoCache
    local getInfoKeyFromName = BPP_GetInfoKeyFromName
    local strfind = string.find
    local strsub = string.sub
    local tonumber = tonumber
    local ipairs = ipairs
        ...
        if characterData.Kills then
            for nameWithLevel, killData in pairs(characterData.Kills) do
                if killData.kills and killData.kills > 0 then
                    ...
                    if not infoKey then
                        infoKey = BPP_NormalizePlayerName(nameWithoutLevel)
...
function BPP_CalculateHourlyStatistics(charactersToProcess)
    local hourlyData = {}
...
function BPP_CalculateWeekdayStatistics(charactersToProcess)
    local weekdayData = {}
...
function BPP_CalculateMonthlyStatistics(charactersToProcess)
    local monthlyData = {}
...
function BPP_CalculateMonthlyByYearStatistics(charactersToProcess)
    local monthlyByYear = {}
...
function BPP_CalculateYearlyStatistics(charactersToProcess)
    local yearlyData = {}
...
function BPP_GetFrameTitleTextWithCharacterText(titleText)
    if BPP_DB.ShowAccountWideStats then
        titleText = titleText .. " |cFFFFFFFF(account-wide stats)|r"
    else
        titleText = titleText .. " (" .. BPP_GetCharacterKey() .. ")"
    end
    return titleText
end

local function setupMainFrame()
    local frame = CreateFrame("Frame", "BPP_StatisticsFrame", UIParent, "BasicFrameTemplateWithInset")
    ...
    tinsert(UISpecialFrames, "BPP_StatisticsFrame")

    local titleText = BPP_GetFrameTitleTextWithCharacterText("PvP Statistics")
    ...
    frame.CloseButton:SetScript("OnClick", function()
        BPP_FrameManager:HideFrame("Statistics")
    end)
...
local function createEmptyStatsFrame()
    ...
    local titleText = BPP_GetFrameTitleTextWithCharacterText("Player Kill Statistics")
    frame.TitleText:SetText(titleText)
...
function BPP_CreateStatisticsFrame()
    ...
    for i = #UISpecialFrames, 1, -1 do
        if (UISpecialFrames[i] == "BPP_StatisticsFrame") then
    ...
    if not enoughPlayerKillsRecorded() then
        ...
        BPP_FrameManager:RegisterFrame(statisticsFrame, "Statistics")
        return
    end
    ...
    BPP_UpdateStatisticsFrame(statisticsFrame)
end

function BPP_UpdateStatisticsFrame(frame, externalPlayerData)
    if not frame then
        return
    end
    ...
    if isExternalPlayer and playerDisplayName then
        ...
        titleText = BPP_GetFrameTitleTextWithCharacterText("PvP Statistics")
    end
    frame.TitleText:SetText(titleText)
    ...
    if isExternalPlayer then
        ...
        if BPP_DB.ShowAccountWideStats then
            charactersToProcess = BPP_DB.PlayerKillCounts.Characters
        else
            if BPP_DB.PlayerKillCounts.Characters[currentCharacterKey] then
        ...
        weekdayData = BPP_CalculateWeekdayStatistics(charactersToProcess)
        monthlyData = BPP_CalculateMonthlyStatistics(charactersToProcess)
        yearlyData = BPP_CalculateYearlyStatistics(charactersToProcess)
        monthlyByYearData = BPP_CalculateMonthlyByYearStatistics(charactersToProcess)
        stats = BPP_CalculateSummaryStatistics(charactersToProcess)
        deathsByClassData = BPP_CalculateDeathsByClass()
        monthlyClassBuckets, monthlyClassPercentages, monthlyClassTotals = BPP_CalculateMonthlyClassPercentageStatistics(charactersToProcess)
    end
    ...
    local kdByClassData, kdRawData = BPP_CalculateKDByClass(classData, deathsByClassData)
    ...
    if not isExternalPlayer then
        ...
        local trashButton = CreateFrame("Button", nil, buttonContainer, "UIPanelButtonTemplate")
        trashButton:SetSize(buttonWidth, buttonHeight)
        trashButton:SetPoint("TOPRIGHT", killstreakButton, "BOTTOMRIGHT", 0, -buttonSpacing)
        trashButton:SetText("Show Trash")
        ...
        nearbyButton:SetPoint("TOPRIGHT", trashButton, "BOTTOMRIGHT", 0, -buttonSpacing)
...
function BPP_ShowPlayerDetailedStats(playerName, detailedStats)
    -- Create a separate frame for viewing other players' stats
    local viewerFrame = CreateFrame("Frame", "BPP_PlayerStatsViewer_" .. playerName, UIParent, "BasicFrameTemplateWithInset")
    ...
    if BPP_FrameManager then
        BPP_FrameManager:RegisterFrame(viewerFrame, "PlayerStatsViewer_" .. playerName)
    end

    -- Reuse the existing update function with external player data
    BPP_UpdateStatisticsFrame(viewerFrame, detailedStats)
