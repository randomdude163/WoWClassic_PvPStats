...
-- Handles importing data from older addons or manual file copies (BPP_DB_IMPORT)
...
local function ResolveImportInfoKey(unitName)
    ...
    if BPP_DB_IMPORT and BPP_DB_IMPORT.PlayerInfoCache then
        local searchPrefix = unitName .. "-"
        for key, _ in pairs(BPP_DB_IMPORT.PlayerInfoCache) do
    ...
    local _, foundInfoKey = BPP_GetPlayerInfo(unitName)
...
local function NormalizeImportedLocationZones(locations)
    ...
    for _, loc in ipairs(locations) do
        if loc.zone then
            local normalized = BPP_ConvertZoneToEnglish(loc.zone)
...
local function BPP_RunMigrationTaskQueue(taskQueue, onDone)
    local currentTask = 1
...
function BPP_ShowImportSummaryPopup()
    if not BPP_DB or not BPP_DB.ImportSummaryPending then
        return
    end

    local summary = BPP_DB.ImportSummaryPending
    local text = BuildImportSummaryText(summary)
    if not text then
        BPP_DB.ImportSummaryPending = nil
        return
    end

    StaticPopupDialogs["BPP_IMPORT_SUMMARY"] = {
        text = text,
    ...
    StaticPopup_Show("BPP_IMPORT_SUMMARY")
end
...
function BPP_PerformDataMigration(runSync)
    ...
    if BPP_MigratePlayerInfoToEnglish then
    ...
    local function initKillState()
        if not BPP_DB.PlayerKillCounts then BPP_DB.PlayerKillCounts = {} end
        if not BPP_DB.PlayerKillCounts.Characters then BPP_DB.PlayerKillCounts.Characters = {} end
        ...
        if BPP_DB_IMPORT.PlayerKillCounts and BPP_DB_IMPORT.PlayerKillCounts.Characters then
            for charKey, _ in pairs(BPP_DB_IMPORT.PlayerKillCounts.Characters) do
    ...
    local function processKillSlice()
        if not BPP_DB_IMPORT.PlayerKillCounts or not BPP_DB_IMPORT.PlayerKillCounts.Characters then
            return true
        end
        ...
        while processed < killBudget do
            ...
            local charData = BPP_DB_IMPORT.PlayerKillCounts.Characters[charKey]

            if not BPP_DB.PlayerKillCounts.Characters[charKey] then
                BPP_DB.PlayerKillCounts.Characters[charKey] = {
                    Kills = {},
            ...
            if not charData or not charData.Kills then
                ...
                if killState.killIndex > #killState.killKeys then
                    BPP_DB.PlayerKillCounts.Characters[charKey].Level1KillTimestamps = nil

                    local destChar = BPP_DB.PlayerKillCounts.Characters[charKey]
                    ...
                    if sourceEntry then
                        local added = MergeKillEntry(BPP_DB.PlayerKillCounts.Characters[charKey].Kills, unitKey, sourceEntry, killState.counters)
    ...
    local function initLossState()
        if not BPP_DB.PvPLossCounts then BPP_DB.PvPLossCounts = {} end
        ...
        if BPP_DB_IMPORT.PvPLossCounts then
            for charKey, _ in pairs(BPP_DB_IMPORT.PvPLossCounts) do
    ...
    local function processLossSlice()
        if not BPP_DB_IMPORT.PvPLossCounts then
            return true
        end
        ...
        while processed < lossBudget do
            ...
            local charData = BPP_DB_IMPORT.PvPLossCounts[charKey]

            if not BPP_DB.PvPLossCounts[charKey] then
                BPP_DB.PvPLossCounts[charKey] = { Deaths = {} }
            end
            ...
            if not charData or not charData.Deaths then
                ...
                if lossState.deathIndex > #lossState.deathKeys then
                    ...
                    if sourceEntry then
                        MergeLossEntry(BPP_DB.PvPLossCounts[charKey].Deaths, unitName, sourceEntry, lossState.counters)
                    end
                    processed = processed + 1
                end
            end
    ...
    local function mergeAchievements()
        if BPP_DB_IMPORT.CharacterAchievements then
            if not BPP_DB.CharacterAchievements then BPP_DB.CharacterAchievements = {} end

            for charKey, achievements in pairs(BPP_DB_IMPORT.CharacterAchievements) do
                if not BPP_DB.CharacterAchievements[charKey] then
                    BPP_DB.CharacterAchievements[charKey] = {}
                end
                MergeAchievements(BPP_DB.CharacterAchievements[charKey], achievements)

                if BPP_UpdateTotalAchievementPoints then
    ...
    local function finalizeMigration()
        ...
        StaticPopupDialogs["BPP_IMPORT_RELOAD"] = {
            text = "BigPPvPStats: Please reload the UI to complete the import.",
            button1 = "Reload UI",
            OnAccept = function()
                ReloadUI()
            end,
        ...
        StaticPopup_Show("BPP_IMPORT_RELOAD")
    end
    ...
    if runSync then
        ...
        BPP_RunMigrationTaskQueue(taskQueue)
    end
end

function BPP_CheckForDataMigration()
    if not BPP_DB_IMPORT then return end
    ...
    if BPP_DB_IMPORT.PlayerKillCounts and BPP_DB_IMPORT.PlayerKillCounts.Characters then
        for charKey, charData in pairs(BPP_DB_IMPORT.PlayerKillCounts.Characters) do
            ...
            local achCount = 0
            if BPP_DB_IMPORT.CharacterAchievements and BPP_DB_IMPORT.CharacterAchievements[charKey] then
                for _, aData in pairs(BPP_DB_IMPORT.CharacterAchievements[charKey]) do
    ...
    local text = "BigPPvPStats: Data ready for import!\n\n"
    ...
    StaticPopupDialogs["BPP_IMPORT_CONFIRM"] = {
        text = text,
        button1 = "Yes, Import",
        button2 = "No, Discard data",
        OnAccept = function()
            BPP_PerformDataMigration(false)
        end,
        OnCancel = function()
            BPP_DB_IMPORT = nil
    ...
    StaticPopup_Show("BPP_IMPORT_CONFIRM")
end

-- Test suite for Data Migration
function BPP_RunMigrationTests()
    if not BPP_Debug then
    ...
    -- Note: DataStorage.lua uses BPP_GetInfoKeyFromName which defaults to BPP_RealmName if logic fails.
    -- In unit tests, we don't control BPP_RealmName easily unless we mock it or rely on caching.
    ...
    -- 2. Swap Globals
    local real_DB = BPP_DB
    local real_Import = BPP_DB_IMPORT
    local real_ReloadUI = ReloadUI

    BPP_DB = mock_DB
    BPP_DB_IMPORT = mock_Import

    local reloadCalled = false
    ReloadUI = function() reloadCalled = true end

    -- 3. Run Migration
    -- Note: We do NOT use BPP_CheckForDataMigration because that creates a popup.
    -- We call BPP_PerformDataMigration directly.
    BPP_PerformDataMigration(true)
    ...
    -- InfoCache
    Assert(BPP_DB.PlayerInfoCache["Old-Realm"], "Case A: Imported InfoCache missing")
    Assert(BPP_DB.PlayerInfoCache["Current-Realm"], "Case B: Existing InfoCache missing")
    Assert(BPP_DB.PlayerInfoCache["Twink-Realm"].level == 29, "Case C: Existing InfoCache overwritten (level mismatch)")
    Assert(BPP_DB.PlayerInfoCache["Note-Target"].note == "Imported Note", "Case D: Note not merged")

    -- Kills
    local charStats = BPP_DB.PlayerKillCounts.Characters[charKey]
    ...
    Assert(next(BPP_DB.LeaderboardCache) == nil, "LeaderboardCache was imported (should be ignored)")
    ...
    Assert(BPP_DB.PvPLossCounts[charKey].Deaths["KillerA"], "Existing killer should persist")
    Assert(BPP_DB.PvPLossCounts[charKey].Deaths["KillerA"].deaths == 2, "Merged killer should have 2 deaths (1 existing + 1 new - 1 duplicate)")
    Assert(BPP_DB.PvPLossCounts[charKey].Deaths["KillerB"], "New killer should be added")

    -- Assertions: Achievements
    print("[PvPStats - Test] Checking Achievements...")
    Assert(BPP_DB.CharacterAchievements[charKey][2], "New achievement should be added")
    Assert(BPP_DB.CharacterAchievements[charKey][1].completedDate == "CurrentDate", "Existing achievement should not be overwritten by older import")
    Assert(BPP_DB.CharacterAchievements[charKey][3].unlocked == true, "Achievement should upgrade from Locked to Unlocked")

    -- Reload & Cleanup
    Assert(reloadCalled, "ReloadUI not triggered")
    Assert(BPP_DB_IMPORT == nil, "BPP_DB_IMPORT not cleared")

    -- 5. Restore Globals
    BPP_DB = real_DB
    BPP_DB_IMPORT = real_Import
