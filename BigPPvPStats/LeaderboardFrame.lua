...
BPP_LeaderboardFrame = nil

BPP_SortLeaderboardBy = "totalKills"
BPP_SortLeaderboardAscending = false
local LEADERBOARD_FRAME_WIDTH = 1150
local LEADERBOARD_FRAME_HEIGHT = 550

BPP_LeaderboardFrameInitialSetup = true

local BPP_LeaderboardDataCache = nil
local BPP_LeaderboardRowDropDown = nil
local BPP_FilterRecentSyncOnly = false
local BPP_ShowOwnAlts = true
...
local function GetLeaderboardEntryUniqueName(entry)
    ...
    if not string.find(name, "%-") then
        local realmName = entry.realm or BPP_RealmName
...
local function IsLocalLeaderboardEntry(entry)
    ...
    local localRealm = BPP_RealmName
...
local function RemoveLeaderboardEntry(entryOrName)
    ...
    if BPP_DB and BPP_DB.LeaderboardCache then
        for name, _ in pairs(candidates) do
            BPP_DB.LeaderboardCache[name] = nil
        end
    end

    BPP_LeaderboardDataCache = nil
    BPP_RefreshLeaderboardFrame()
end

local function RemoveOfflineLeaderboardEntries()
    if not BPP_DB or not BPP_DB.LeaderboardCache then return end
    ...
    if BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters then
        for characterKey, _ in pairs(BPP_DB.PlayerKillCounts.Characters) do
    ...
    for cacheName, _ in pairs(BPP_DB.LeaderboardCache) do
        ...
        if not isLocal and not isLive and not isOwnCharacter then
            BPP_DB.LeaderboardCache[cacheName] = nil
        end
    end
...
local function ApplyLeaderboardFilters(entries)
    if not BPP_FilterRecentSyncOnly then
        return entries
    end
...
if not StaticPopupDialogs["BPP_REMOVE_OFFLINE_LEADERBOARD"] then
    StaticPopupDialogs["BPP_REMOVE_OFFLINE_LEADERBOARD"] = {
...
if not StaticPopupDialogs["BPP_SEND_STATS_TO_PLAYER"] then
    StaticPopupDialogs["BPP_SEND_STATS_TO_PLAYER"] = {
        ...
        OnAccept = function(self)
            ...
            if playerName and playerName ~= "" then
                ...
                if not normalized or normalized == "" then
                    BPP_Print("Invalid player name.")
                    return
                end

                local sent, _, reason = PVPSC.Network:SendStatsToPlayer(normalized)
                if not sent then
                    BPP_Print("Failed to send stats: " .. (reason or "Unknown error"))
                else
                    BPP_Print("Stats sent to " .. normalized .. ". They will only receive them if they have addon version 4.2 or higher.")
                end
            end
        end,
...
local function CreateColumnHeader(parent, text, width, anchor, xOffset, yOffset, columnId)
    ...
    button:SetScript("OnClick", function()
        if BPP_SortLeaderboardBy == columnId then
            BPP_SortLeaderboardAscending = not BPP_SortLeaderboardAscending
        else
            BPP_SortLeaderboardBy = columnId
            BPP_SortLeaderboardAscending = false
        end
        BPP_RefreshLeaderboardFrame(true)
    end)
    ...
    if BPP_SortLeaderboardBy == columnId then
        local sortIndicator = BPP_SortLeaderboardAscending and " ^" or " v"
...
local function CreateLastSeenCell(content, anchorTo, timestamp, width)
    ...
    local timeString = BPP_GetTimeAgo(timestamp, true)
...
local function CreateEntryRow(content, entry, yOffset, colWidths, isAlternate)
    ...
    if entry.hasDetailedStats or entry.playerName == UnitName("player") then
        local highlightTexture = BPP_CreateGoldHighlight(rowContainer, 16)
    end
            ...
            if entry.hasDetailedStats then
                ...
                if PVPSC.Network and PVPSC.Network.GetDetailedStatsForPlayer then
                    ...
                    if detailedStats then
                        ...
                        BPP_ShowPlayerDetailedStats(entry.playerName, detailedStats)
                    end
                end
            end
        end
    ...
    rowContainer:SetScript("OnMouseDown", function(self, button)
        if button == "RightButton" then
            ...
            if not BPP_LeaderboardRowDropDown then
                BPP_LeaderboardRowDropDown = CreateFrame("Frame", "BPP_LeaderboardRowDropDown", UIParent, "UIDropDownMenuTemplate")
            end

            UIDropDownMenu_Initialize(BPP_LeaderboardRowDropDown, function(self, level)
            ...
            ToggleDropDownMenu(1, nil, BPP_LeaderboardRowDropDown, self, 0, 0)
            BPP_LeaderboardRowDropDown._justOpened = true
            C_Timer.After(0, function()
                if BPP_LeaderboardRowDropDown then
                    BPP_LeaderboardRowDropDown._justOpened = nil
                end
            end)
            return
        end
...
local function GetLeaderboardData()
    ...
    if PVPSC.Network and PVPSC.Network.GetAllLeaderboardData then
        netData = PVPSC.Network:GetAllLeaderboardData(BPP_ShowOwnAlts)
    end
    ...
    if netData and #netData > 0 then
         ...
         for _, data in ipairs(netData) do
             ...
             local kdRatioVal = BPP_FormatKDRatio(kills, deaths)
             ...
             local realmName = data.realm or BPP_RealmName
...
local function SortLeaderboardData(data)
    ...
    table.sort(sortedData, function(a, b)
        ...
        if BPP_SortLeaderboardBy == "mostKilled" then
            local aCount = a._mostKilledCount or 0
            local bCount = b._mostKilledCount or 0
            if aCount ~= bCount then
                if BPP_SortLeaderboardAscending then
        ...
        if BPP_SortLeaderboardBy == "addonVersion" then
            local cmp = BPP_CompareAddonVersions(a.addonVersion, b.addonVersion)
            if cmp ~= 0 then
                if BPP_SortLeaderboardAscending then
                    return cmp < 0
                else
        ...
        local aVal = a[BPP_SortLeaderboardBy]
        local bVal = b[BPP_SortLeaderboardBy]
...
local function DisplayEntries(content, sortedEntries, yOffset)
    ...
    if #sortedEntries == 0 then
        ...
        if BPP_FilterRecentSyncOnly then
...
local function CreateMainFrame()
    local frame = CreateFrame("Frame", "BPP_LeaderboardStatsFrame", UIParent, "BasicFrameTemplateWithInset")
    ...
    if frame.CloseButton then
        frame.CloseButton:SetScript("OnClick", function()
            BPP_FrameManager:HideFrame("Leaderboard")
        end)
    end
...
function BPP_RefreshLeaderboardFrame(useCache)
    if BPP_LeaderboardFrameInitialSetup then
        return
    end

    if BPP_LeaderboardFrame == nil then
        return
    end

    local content = BPP_LeaderboardFrame.content
    if not content then
        return
    end
    ...
    local leaderboardData
    if useCache and BPP_LeaderboardDataCache then
        leaderboardData = BPP_LeaderboardDataCache
    else
        leaderboardData = GetLeaderboardData()
        BPP_LeaderboardDataCache = leaderboardData
    end
...
function BPP_CreateLeaderboardFrame()
    if (BPP_LeaderboardFrame) then
        BPP_FrameManager:ShowFrame("Leaderboard")
        BPP_RefreshLeaderboardFrame()
        return
    end

    BPP_LeaderboardFrame = CreateMainFrame()
    BPP_LeaderboardFrame.content = CreateScrollFrame(BPP_LeaderboardFrame)

    local function CloseLeaderboardContextMenu()
        if BPP_LeaderboardRowDropDown and BPP_LeaderboardRowDropDown:IsShown() then
    ...
    BPP_LeaderboardFrame:SetScript("OnHide", function()
        CloseLeaderboardContextMenu()
    end)

    BPP_LeaderboardFrame:SetScript("OnMouseDown", function()
        CloseLeaderboardContextMenu()
    end)

    if BPP_LeaderboardFrame.content then
        BPP_LeaderboardFrame.content:SetScript("OnMouseDown", function()
            CloseLeaderboardContextMenu()
        end)
    end

    local removeOfflineButton = CreateFrame("Button", nil, BPP_LeaderboardFrame, "UIPanelButtonTemplate")
    removeOfflineButton:SetSize(170, 25)
    removeOfflineButton:SetPoint("BOTTOMLEFT", BPP_LeaderboardFrame, "BOTTOMRIGHT", -197, 10)
    removeOfflineButton:SetText("Remove offline players")
    removeOfflineButton:SetScript("OnClick", function()
        StaticPopup_Show("BPP_REMOVE_OFFLINE_LEADERBOARD")
    end)
    BPP_LeaderboardFrame.removeOfflineButton = removeOfflineButton

    local sendStatsButton = CreateFrame("Button", nil, BPP_LeaderboardFrame, "UIPanelButtonTemplate")
    ...
    sendStatsButton:SetScript("OnClick", function()
        StaticPopup_Show("BPP_SEND_STATS_TO_PLAYER")
    end)
    ...
    recentSyncCheckbox:SetChecked(BPP_FilterRecentSyncOnly)

    if BPP_DB.ShowOwnAltsOnLeaderboard == nil then
        BPP_DB.ShowOwnAltsOnLeaderboard = true
    end
    BPP_ShowOwnAlts = BPP_DB.ShowOwnAltsOnLeaderboard

    local showAltsCheckbox = CreateFrame("CheckButton", nil, BPP_LeaderboardFrame, "UICheckButtonTemplate")
    showAltsCheckbox:SetSize(20, 20)
    showAltsCheckbox:SetPoint("RIGHT", recentSyncCheckbox, "LEFT", -88, 0)
    showAltsCheckbox:SetChecked(BPP_ShowOwnAlts)

    local showAltsLabel = BPP_LeaderboardFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    ...
    local recentSyncLabel = BPP_LeaderboardFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    ...
    local infoText = BPP_LeaderboardFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    ...
    BPP_LeaderboardFrame.infoText = infoText
    ...
    showAltsCheckbox:SetScript("OnClick", function(self)
        BPP_ShowOwnAlts = self:GetChecked()
        BPP_DB.ShowOwnAltsOnLeaderboard = BPP_ShowOwnAlts
        BPP_LeaderboardDataCache = nil
        BPP_RefreshLeaderboardFrame()
    end)
    ...
    recentSyncCheckbox:SetScript("OnClick", function(self)
        BPP_FilterRecentSyncOnly = self:GetChecked()
        BPP_RefreshLeaderboardFrame(true)
    end)
    ...
    BPP_LeaderboardFrame.recentSyncCheckbox = recentSyncCheckbox
    BPP_LeaderboardFrame.recentSyncLabel = recentSyncLabel
    BPP_LeaderboardFrame.showAltsCheckbox = showAltsCheckbox
    BPP_LeaderboardFrame.showAltsLabel = showAltsLabel

    BPP_FrameManager:RegisterFrame(BPP_LeaderboardFrame, "Leaderboard")

    for i = #UISpecialFrames, 1, -1 do
        if (UISpecialFrames[i] == "BPP_LeaderboardStatsFrame") then
    ...
    C_Timer.After(0.01, function()
        BPP_LeaderboardFrameInitialSetup = false
        BPP_RefreshLeaderboardFrame()
    end)
end
