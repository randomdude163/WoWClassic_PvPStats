...
local function BPP_CreateKillMilestoneFrame()
    if killMilestoneFrame then return killMilestoneFrame end

    local milestoneFrame = CreateFrame("Frame", "BPP_KillMilestoneFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate")
...
function BPP_ShowKillMilestone(playerName, level, class, rank, killCount)
    if not BPP_DB.ShowKillMilestones then return end
    if not BPP_DB.ShowMilestoneForFirstKill and killCount == 1 then return end

    local milestoneFrame = BPP_CreateKillMilestoneFrame()
    local class_upper = class:upper()

    local pos = BPP_DB.MilestoneFramePosition
    milestoneFrame:ClearAllPoints()
    milestoneFrame:SetPoint(pos.point, UIParent, pos.relativePoint, pos.xOfs, pos.yOfs)

    if killCount ~= 1 and killCount % BPP_DB.KillMilestoneInterval ~= 0 then
        return
    end
    ...
    milestoneFrame:Show()
    local animGroup = BPP_SetupKillstreakMilestoneAnimation(milestoneFrame, BPP_DB.KillMilestoneAutoHideTime)
    animGroup:Play()

    if BPP_DB.EnableKillMilestoneSound then
    ...
    killMilestoneAutoHideTimer = C_Timer.NewTimer(BPP_DB.KillMilestoneAutoHideTime + 1.0, function()
        milestoneFrame:Hide()
        killMilestoneAutoHideTimer = nil
    end)
end
