...
BPP_UnattributedPetDamage = {}

-- Create a hidden tooltip to scan pet information (reused)
local scanTooltip = CreateFrame("GameTooltip", "BPP_PetScanTooltip", nil, "GameTooltipTemplate")
scanTooltip:SetOwner(WorldFrame, "ANCHOR_NONE")

-- This function extracts pet owner information from tooltip for any unit
function BPP_UpdatePetOwnerFromUnit(unitID)
    ...
    if BPP_PetOwnerCache[petName] then
        return BPP_PetOwnerCache[petName]
    end
    ...
    for i = 1, scanTooltip:NumLines() do
        local line = _G["BPP_PetScanTooltipTextLeft" .. i]:GetText()
        if line then
            ...
            if owner then
                ...
                BPP_PetOwnerCache[petName] = owner

                -- Check if we have unattributed damage from this pet
                if BPP_UnattributedPetDamage[petName] then
                    local petInfo = BPP_UnattributedPetDamage[petName]

                    -- Create a name-based key for the owner
                    local ownerName = owner

                    -- Get or create owner record
                    local existingRecord = BPP_RecentDamageFromPlayers[ownerName] or {
                        name = owner,
                        class = "Unknown",
                        totalDamage = 0,
                        timestamp = 0
                    ...
                    BPP_RecentDamageFromPlayers[ownerName] = existingRecord
                    ...
                    BPP_UnattributedPetDamage[petName] = nil
                end
...
function BPP_GetKillerInfoOnDeath()
    local now = GetTime()
    local killers = {}
    local mainKiller = nil
    local highestDamage = 0

    for sourceID, info in pairs(BPP_RecentDamageFromPlayers) do
...
function BPP_RegisterPlayerDeath(killerInfo)
    ...
    local killerInfoKey = BPP_GetInfoKeyFromName(killerName)

    local killerLevel = -1
    if BPP_DB.PlayerInfoCache[killerInfoKey] then
        killerLevel = BPP_DB.PlayerInfoCache[killerInfoKey].level
    end
    ...
    deathData.zone = BPP_GetCurrentZoneName()
    ...
    local x, y = BPP_GetPlayerCoordinates()
    ...
    if #killerInfo.assists > 0 then
        ...
        for _, assist in ipairs(killerInfo.assists) do
            ...
            local assisterInfoKey = BPP_GetInfoKeyFromName(assist.name)

            -- Capture assister's level at time of kill from cache if available
            if BPP_DB.PlayerInfoCache[assisterInfoKey] then
                assisterLevel = BPP_DB.PlayerInfoCache[assisterInfoKey].level
                assisterClass = BPP_DB.PlayerInfoCache[assisterInfoKey].class
            end
...
function BPP_TrackIncomingPlayerDamage(sourceGUID, sourceName, amount)
    ...
    local existingRecord = BPP_RecentDamageFromPlayers[playerKey] or {
        name = sourceName,
    ...
    BPP_RecentDamageFromPlayers[playerKey] = existingRecord
...
-- Fix for the BPP_TrackIncomingPetDamage function
function BPP_TrackIncomingPetDamage(petGUID, petName, amount)
    if not petGUID or not petName then return end

    -- First check our cache for the pet owner
    local ownerName = BPP_PetOwnerCache[petName]

    -- If we couldn't find the owner in our cache
    if not ownerName then
        -- Store the damage with the pet temporarily
        BPP_UnattributedPetDamage[petName] = BPP_UnattributedPetDamage[petName] or {
            totalDamage = 0,
            timestamp = GetTime()
        }

        -- Update unattributed damage
        BPP_UnattributedPetDamage[petName].totalDamage = BPP_UnattributedPetDamage[petName].totalDamage + amount
        BPP_UnattributedPetDamage[petName].timestamp = GetTime()
    ...
    -- Use the owner's name directly as the key for consistency with BPP_TrackIncomingPlayerDamage
...
function BPP_HandleReceivedPlayerDamage(combatEvent, sourceGUID, sourceName, spellId, spellName, param1, param4)
    ...
    if damageAmount > 0 then
        BPP_TrackIncomingPlayerDamage(sourceGUID, sourceName, damageAmount)
    end
end

function BPP_HandleReceivedPlayerDamageByEnemyPets(combatEvent, sourceGUID, sourceName, param1, param4)
    local damageAmount = 0
    ...
    if damageAmount > 0 then
        BPP_TrackIncomingPetDamage(sourceGUID, sourceName, damageAmount)
    end
end

function BPP_CleanupUnattributedPetDamage()
    BPP_UnattributedPetDamage = {}
end

function BPP_CleanupRecentDamageFromPlayers()
    BPP_RecentDamageFromPlayers = {}

    BPP_CleanupUnattributedPetDamage()
end

function BPP_ShowDeathStats()
    local characterKey = BPP_GetCharacterKey()
    if not BPP_DB.PvPLossCounts or not BPP_DB.PvPLossCounts[characterKey] then
        BPP_Print("No death data available")
        return
    end

    local lossData = BPP_DB.PvPLossCounts[characterKey]
    BPP_Print("Death Stats for " .. BPP_CharacterName)
    BPP_Print("\nDeaths by player:")
    ...
    for i, death in ipairs(deaths) do
        local dateStr = BPP_FormatTimestamp(death.lastDeath)
        BPP_Print(i .. ". " .. death.name .. " - " .. death.total ..
