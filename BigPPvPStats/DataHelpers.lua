-- Common data helper functions for BigPPvPStats
-- These functions centralize common data operations used throughout the addon

function BPP_GetCurrentZoneName()
    ...
    if IsValidZoneName(realZone) then
        return BPP_ConvertZoneToEnglish(realZone)
    end

    local minimapZone = GetMinimapZoneText()
    if IsValidZoneName(minimapZone) then
        return BPP_ConvertZoneToEnglish(minimapZone)
    end

    local zoneText = GetZoneText()
    if IsValidZoneName(zoneText) then
        return BPP_ConvertZoneToEnglish(zoneText)
    end

    local subZone = GetSubZoneText()
    if IsValidZoneName(subZone) then
        return BPP_ConvertZoneToEnglish(subZone)
    end

    local instanceName, instanceType = GetInstanceInfo()

    if IsValidZoneName(instanceName) then
        return BPP_ConvertZoneToEnglish(instanceName)
    end
...
function BPP_GetDeathDataFromAllCharacters(characterScope)
    ...
    elseif BPP_DB.ShowAccountWideStats then
...
function BPP_GetPlayerMostRecentDeathInfo(deathData)
    local zone = "Unknown"
    local lastKill = 0
...
function BPP_CountPlayerAssists(playerName, deathDataByPlayer)
    local assistCount = 0
    local assistData = {
        assists = 0,
        lastAssist = 0,
...
function BPP_SortPlayerEntries(entries, sortBy, ascending)
    if sortBy then
...
function BPP_AddOrUpdatePlayerEntry(playerNameMap, entries, name, entry)
