...
## Step 1: Backup
Before proceeding, locate your TBC Client's `SavedVariables` folder and create a backup of your `BigPPvPStats.lua`. Open Battle.net launcher and click *Show in Explorer* for TBC Anniversary:
...
Navigate to `WTF\Account\<your account>\SavedVariables` and create a backup of your `BigPPvPStats.lua`file:
...
## Step 3: Find the Classic Era `BigPPvPStats.lua` file
Navigate to `WTF\Account\<your account>\SavedVariables` and select the file `BigPPvPStats.lua`
...
In the opened file, find the line that starts with `BPP_DB = {`.
**Rename** `BPP_DB` to `BPP_DB_IMPORT`.
...
## Step 6: Open TBC Anniversary `BigPPvPStats.lua` file
Go back to the TBC Anniversary `SavedVariables` folder (where you created the backup in Step 1) and open the file `BigPPvPStats.lua` in Notepad:
...
Paste the copied content at the very **beginning** of the file (before the existing `BPP_DB` block).
...
Open the statistics window (left click on the minimap button or type */bpp statistics* in the chat) and look at all the numbers and charts. Do they look plausible? If yes, you are done. If no, close the game, delete the `BigPPvPStats.lua` file in the TBC Anniversary `SavedVariables` folder, copy your backup file and name it `BigPPvPStats.lua`, then start again from Step 1.
