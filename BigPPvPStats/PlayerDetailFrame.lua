...
local RACE_ICON_IDS = {
    ["HUMAN_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236448",
    ["HUMAN_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236447",
    ["DWARF_MALE"] = 236444,
    ["DWARF_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236443",
    ["GNOME_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236446",
    ["GNOME_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236445",
    ["NIGHTELF_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236450",
    ["NIGHTELF_FEMALE"] = 236449,
    ["ORC_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236452",
    ["ORC_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236451",
    ["TAUREN_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236454",
    ["TAUREN_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236453",
    ["TROLL_MALE"] = 236456,
    ["TROLL_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236455",
    ["UNDEAD_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236458",
    ["UNDEAD_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236457",
    ["SCOURGE_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236458",
    ["SCOURGE_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236457",
    ["BLOODELF_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236440",
    ["BLOODELF_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236439",
    ["DRAENEI_MALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236442",
    ["DRAENEI_FEMALE"] = "Interface\\AddOns\\BigPPvPStats\\img\\icons\\236441"
}

function BPP_FormatTimestamp(timestamp)
...
local function CreateKillHistoryHeaderRow(content, yOffset)
    ...
    levelHeader:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.LEVEL, yOffset - 3)
...
local function CreateKillHistoryEntry(parent, killData, index, yOffset)
    ...
    levelText:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.LEVEL, yOffset - 3)
    levelText:SetText(killData.level == -1 and "??" or tostring(killData.level))
    levelText:SetWidth(BPP_COLUMN_WIDTHS.LEVEL)

    local zoneText = parent:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    zoneText:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.ZONE, yOffset - 3)
    zoneText:SetText(killData.zone or "Unknown")
    zoneText:SetWidth(BPP_COLUMN_WIDTHS.ZONE)
    ...
    yourLevelText:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.KILLS, yOffset - 3)
    ...
    yourLevelText:SetWidth(BPP_COLUMN_WIDTHS.KILLS)
    yourLevelText:SetJustifyH("LEFT")

    local timeText = parent:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    timeText:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.TIME, yOffset - 3)
    timeText:SetText(BPP_FormatTimestamp(killData.timestamp))
...
local function CreateDeathHistoryEntry(parent, deathData, index, yOffset)
    ...
    assistFrame:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.KILLS, yOffset - 3)
    ...
    assistText:SetWidth(BPP_COLUMN_WIDTHS.KILLS)
    ...
    local timeText = parent:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    timeText:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.TIME, yOffset - 3)
    timeText:SetText(BPP_FormatTimestamp(deathData.timestamp))
...
function BPP_CreatePlayerDetailInfo(playerName)
    ...
    local playerInfo = BPP_GetPlayerInfo(playerName)
    ...
    for charKey, charData in pairs(charactersToProcess) do
        for nameWithLevel, killData in pairs(charData.Kills or {}) do
            ...
            if name and BPP_IsSamePlayerName(name, playerName) then
    ...
    local deathDataByPlayer = BPP_GetDeathDataFromAllCharacters()
    local function CollectDeathDataForPlayer(targetName)
        ...
        for killerName, deathData in pairs(deathDataByPlayer) do
            if BPP_IsSamePlayerName(killerName, targetName) then
                found = true
    ...
    local assistCount, _ = BPP_CountPlayerAssists(playerName, deathDataByPlayer)
            ...
            for _, location in ipairs(deathData.deathLocations) do
                if location.assisters then
                    for _, assister in ipairs(location.assisters) do
                        if BPP_IsSamePlayerName(assister.name, playerName) then
                            ...
                            local assistData = {
                                ...
                                killerClass = BPP_GetPlayerInfo(killerName).class or "Unknown",
                            ...
                            for _, otherAssister in ipairs(location.assisters) do
                                if not BPP_IsSamePlayerName(otherAssister.name, playerName) then
...
local function CreatePlayerDetailFrame()
    local frame = CreateFrame("Frame", "BPP_PlayerDetailFrame", UIParent, "BasicFrameTemplateWithInset")
    ...
    if frame.CloseButton then
        frame.CloseButton:SetScript("OnClick", function()
            BPP_FrameManager:HideFrame("PlayerDetail")
        end)
    end
...
local function DisplayPlayerSummarySection(content, playerDetail, yOffset)
    ...
    local isFromDifferentRealm, cleanPlayerName, playerRealm = BPP_IsPlayerFromDifferentRealm(playerDetail.name)
    ...
    if totalEncounters > 0 then
        ...
        local winPercentageColor = BPP_GetWinPercentageColor(winPct)
    ...
    if BPP_PlayerDetailFrame then
        BPP_PlayerDetailFrame.activeNoteEditBox = noteEditBox
    end

    if not BPP_DB then BPP_DB = {} end
    if not BPP_DB.PlayerInfoCache then BPP_DB.PlayerInfoCache = {} end

    -- Try to find existing entry (possibly from another realm)
    local playerCacheEntry, entryKey = BPP_GetPlayerInfo(playerDetail.name)

    -- If no entry found, create one for current realm
    if not entryKey then
        entryKey = BPP_GetInfoKeyFromName(playerDetail.name)
        BPP_DB.PlayerInfoCache[entryKey] = {}
        playerCacheEntry = BPP_DB.PlayerInfoCache[entryKey]
    end
        ...
        if newText ~= (playerCacheEntry.note or "") then
            ...
            for targetPlayerName in string.gmatch(newText, "@([^@%s]+)") do
                if targetPlayerName and targetPlayerName ~= currentCharacterName then
                    ...
                    local targetInfoKey = BPP_GetInfoKeyFromName(targetPlayerName)
                    if targetInfoKey then
                        if not BPP_DB.PlayerInfoCache[targetInfoKey] then
                            BPP_DB.PlayerInfoCache[targetInfoKey] = {}
                        end
                        BPP_DB.PlayerInfoCache[targetInfoKey].note = linkedNoteText
                    end
                end
            end
        end
...
local function DisplayKillHistorySection(content, playerDetail, yOffset)
    ...
    yourLevelHeader:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.KILLS, yOffset - 3)
    yourLevelHeader:SetText("Your Level")
    yourLevelHeader:SetTextColor(1, 0.82, 0)
    yourLevelHeader:SetWidth(BPP_COLUMN_WIDTHS.KILLS)
...
local function CreateDeathHistoryHeaderRow(content, yOffset)
    ...
    killerLevelHeader:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.LEVEL, yOffset - 3)
...
local function CreateAssistHistoryHeaderRow(content, yOffset)
    ...
    victimLevelHeader:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.LEVEL, yOffset - 3)
...
local function CreateAssistHistoryEntry(parent, assistData, index, yOffset)
    ...
    for _, otherAssister in ipairs(assistData.otherAssisters) do
        if BPP_IsSamePlayerName(otherAssister.name, assistData.assisterName) then
    ...
    killerFrame:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.KILLS, yOffset - 3)
    ...
    local isDifferentRealm, cleanName = BPP_IsPlayerFromDifferentRealm(displayKillerName)
    if cleanName then
        displayKillerName = cleanName .. (isDifferentRealm and "*" or "")
    end

    killerText:SetText(displayKillerName)
    killerText:SetWidth(BPP_COLUMN_WIDTHS.KILLS)
    killerText:SetJustifyH("LEFT")

    -- Color the killer name based on class if known
    local killerInfo = BPP_GetPlayerInfo(assistData.killerName)
    ...
    killerFrame:SetScript("OnEnter", function(self)
        ...
        if assistData.otherAssisters and #assistData.otherAssisters > 0 then
            ...
            for _, assister in ipairs(assistData.otherAssisters) do
                local assisterInfo = BPP_GetPlayerInfo(assister.name)
    ...
    local timeText = parent:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    timeText:SetPoint("TOPLEFT", BPP_COLUMN_POSITIONS.TIME, yOffset - 3)
    timeText:SetText(BPP_FormatTimestamp(assistData.timestamp))
...
local function GetAssistHistoryForPlayer(playerName)
    ...
    if BPP_DB.ShowAccountWideStats then
        ...
        if BPP_DB.PvPLossCounts then
            for cKey, charData in pairs(BPP_DB.PvPLossCounts) do
        ...
        local lossData = BPP_DB.PvPLossCounts[characterKey]
...
function BPP_ShowPlayerDetailFrame(playerName, focusNote)
    if not playerName then return end

    local playerDetail = BPP_CreatePlayerDetailInfo(playerName)
    if not playerDetail then
        print("Could not find detailed information for player:", playerName)
        return
    end

    if not BPP_PlayerDetailFrame then
        BPP_PlayerDetailFrame = CreatePlayerDetailFrame()
        BPP_FrameManager:RegisterFrame(BPP_PlayerDetailFrame, "PlayerDetail")
    else
        CleanupPlayerDetailFrame(BPP_PlayerDetailFrame.content)
    end

    if BPP_PlayerDetailFrame then
        BPP_PlayerDetailFrame.activeNoteEditBox = nil
    end

    local content = BPP_PlayerDetailFrame.content
    local isFromDifferentRealm, cleanPlayerName, playerRealm = BPP_IsPlayerFromDifferentRealm(playerName)
    ...
    BPP_PlayerDetailFrame.TitleText:SetText(titleText)

    BPP_FrameManager:ShowFrame("PlayerDetail")
    ...
    if focusNote and BPP_PlayerDetailFrame and BPP_PlayerDetailFrame.activeNoteEditBox then
        BPP_PlayerDetailFrame.activeNoteEditBox:SetFocus()
        BPP_PlayerDetailFrame.activeNoteEditBox:HighlightText(0, -1)
    end
