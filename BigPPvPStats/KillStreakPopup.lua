...
local function CreatePopupFrame()
    local frame = CreateFrame("Frame", "BPP_KillStreakPopupFrame", UIParent, "BasicFrameTemplateWithInset")
    frame:SetSize(220, 140)

    -- Set position from saved settings or default to bottom right
    local pos = BPP_DB.KillStreakPopupPosition or {point = "BOTTOMRIGHT", relativePoint = "BOTTOMRIGHT", xOfs = -50, yOfs = 150}
    ...
    if frame.CloseButton then
        frame.CloseButton:SetScript("OnClick", function()
            BPP_FrameManager:HideFrame("BPP_KillStreakPopupFrame")
        end)
    end
...
local function CreatePlayerRow(parent, playerData, yOffset, isAlternate)
    ...
    rowButton:SetScript("OnClick", function(self, button)
        if button == "LeftButton" then
            BPP_ShowPlayerDetailFrame(playerData.name)
        end
    end)
...
function BPP_CreateKillStreakPopup(isAutoOpen)
...
function BPP_UpdateKillStreakPopup()
