...
BPP_ZONE_TRANSLATIONS_CLASSIC = {
    ...
    ["Searing Gorge"] = {"Searing Gorge", "Sengende Schlucht", "Gorge des Vents brûlants (Searing Gorge)", "La Garganta de Fuego", "Garganta Abrasadora"},
    ...
    ["Blackrock Mountain"] = {"Blackrock Mountain", "Der Schwarzfels", "Mont Blackrock", "Montaña Roca Negra", "Montanha Rocha Negra"},
    ...
    ["Wailing Caverns"] = {"Wailing Caverns", "Die Höhlen des Wehklagens", "Cavernes des Lamentations", "Cuevas de los Lamentos", "Caverna Uivante"},
...
local function BuildZoneTranslationLookup()
    ...
    AddTranslations(BPP_ZONE_TRANSLATIONS_CLASSIC)
    AddTranslations(BPP_ZONE_TRANSLATIONS_TBC)
...
function BPP_GetPlayerInfoKey(name, realm)
    if not realm then
...
function BPP_GetPlayerInfo(playerName)
    ...
    if not string.find(playerName, "-") then
        ...
        for key, info in pairs(BPP_DB.PlayerInfoCache) do
...
local function InitKillKeyMigrationState()
    ...
    local characters = BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters
    if characters then
        ...
        if not charData or not charData.Kills then
            ...
            if killMigrationState.killIndex > #killMigrationState.killKeys then
                ...
                if name and level then
                    local infoKey
                    local _, existingInfoKey = BPP_GetPlayerInfo(name)

                    if existingInfoKey then
                        infoKey = existingInfoKey
                    else
                        infoKey = BPP_GetInfoKeyFromName(name)
                    end
...
local function InitLossKeyMigrationState()
    ...
    if BPP_DB.PvPLossCounts then
        for charKey in pairs(BPP_DB.PvPLossCounts) do
...
local function ProcessLossKeyMigrationSlice()
    if not BPP_DB.PvPLossCounts then
    ...
    while processed < LOSS_KEY_MIGRATION_BUDGET do
        ...
        local charData = BPP_DB.PvPLossCounts[charKey]
        ...
        if not charData or not charData.Deaths then
            ...
            if lossMigrationState.deathIndex > #lossMigrationState.deathKeys then
                        ...
                        if loc.assisters then
                            for _, assister in ipairs(loc.assisters) do
                                ...
                                if aName and string.find(aName, "-") == nil then
                                    local _, existingInfoKey = BPP_GetPlayerInfo(aName)
                                    if existingInfoKey then
                                        assister.name = existingInfoKey
                                    else
                                        assister.name = BPP_GetInfoKeyFromName(aName)
                                    end
                                end
                            end
                        end
                ...
                if not hasRealm then
                    local _, existingInfoKey = BPP_GetPlayerInfo(oldName)
                    if existingInfoKey then
                        newName = existingInfoKey
                    else
                        newName = BPP_GetInfoKeyFromName(oldName)
                    end
                end
...
function BPP_MigrateLossKeys()
    if BPP_DB.LossKeysMigrated_v3 then return end
...
function BPP_MigrateArenaZones()
    if BPP_DB.ArenaZonesMigrated_v1 then return end
    ...
    -- ── Kills ────────────────────────────────────────────────────────────────
    if BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters then
        for _, charData in pairs(BPP_DB.PlayerKillCounts.Characters) do
    ...
    -- ── Deaths & Assists ─────────────────────────────────────────────────────
    if BPP_DB.PvPLossCounts then
        for _, charLossData in pairs(BPP_DB.PvPLossCounts) do
            if charLossData.Deaths then
                for killerName, deathData in pairs(charLossData.Deaths) do
                    ...
                    local killerLevel = -1
                    if BPP_DB.PlayerInfoCache then
                        local info = BPP_DB.PlayerInfoCache[killerName]
                        if not info then
                            -- Try prefix search ("Cogwhisper" -> "Cogwhisper-Spineshatter")
                            local prefix = killerName .. "-"
                            for cacheKey, cacheInfo in pairs(BPP_DB.PlayerInfoCache) do
    ...
    if not BPP_DB.PlayerInfoCacheMigrated then
        ...
        for name, data in pairs(oldCache) do
            ...
            if not string.find(name, "-") then
                local infoKey = BPP_GetPlayerInfoKey(name)
...
function BPP_ConvertClassToEnglish(localizedClass)
...
function BPP_ConvertRaceToEnglish(localizedRace)
...
function BPP_ConvertZoneToEnglish(localizedZone)
...
function BPP_CompareAddonVersions(v1, v2)
...
function BPP_IsAddonVersionNewer(remoteVersion, localVersion)
    local currentVersion = localVersion
    if currentVersion == nil and BPP_GetAddonVersion then
        currentVersion = BPP_GetAddonVersion()
    end
    ...
    return BPP_CompareAddonVersions(remoteVersion, currentVersion) > 0
end

function BPP_ShowAddonUpdatePopup(remoteVersion)
    if not remoteVersion or remoteVersion == "" then
        return
    end

    if BPP_DB and BPP_DB.LastAddonUpdateNotificationVersion then
        if BPP_CompareAddonVersions(remoteVersion, BPP_DB.LastAddonUpdateNotificationVersion) <= 0 then
            return
        end
    end

    if not StaticPopupDialogs["BPP_ADDON_UPDATE_AVAILABLE"] then
        StaticPopupDialogs["BPP_ADDON_UPDATE_AVAILABLE"] = {
            text = "",
            button1 = "OK",
            timeout = 0,
            whileDead = true,
    ...
    if BPP_DB then
        BPP_DB.LastAddonUpdateNotificationVersion = remoteVersion
    end
end

function BPP_MigratePlayerInfoToEnglish(force)
    if not BPP_DB.PlayerInfoEnglishMigrated or force then
        for _, data in pairs(BPP_DB.PlayerInfoCache) do
            if data.class then
        ...
        local function NormalizeLocationZones(locations)
            ...
            for _, loc in ipairs(locations) do
                if loc.zone then
                    loc.zone = BPP_ConvertZoneToEnglish(loc.zone)
                end
            end
        end
                ...
                if charData.Kills then
                    for _, killData in pairs(charData.Kills) do
                        if killData.zone then
                            killData.zone = BPP_ConvertZoneToEnglish(killData.zone)
                        end
        ...
        if BPP_DB.PvPLossCounts then
            for _, lossData in pairs(BPP_DB.PvPLossCounts) do
                if lossData.Deaths then
                    for _, deathData in pairs(lossData.Deaths) do
                        if deathData.zone then
                            deathData.zone = BPP_ConvertZoneToEnglish(deathData.zone)
                        end
...
function BPP_StorePlayerInfo(name, level, class, race, gender, guildName, guildRankName, rank)
    ...
    local playerNameWithRealm = BPP_GetPlayerInfoKey(name, realm)

    if not BPP_DB.PlayerInfoCache[playerNameWithRealm] then
        BPP_DB.PlayerInfoCache[playerNameWithRealm] = {}
    end

    BPP_DB.PlayerInfoCache[playerNameWithRealm].level = level
    BPP_DB.PlayerInfoCache[playerNameWithRealm].class = class
    BPP_DB.PlayerInfoCache[playerNameWithRealm].race = race
    BPP_DB.PlayerInfoCache[playerNameWithRealm].gender = gender
    BPP_DB.PlayerInfoCache[playerNameWithRealm].guild = guildName
    BPP_DB.PlayerInfoCache[playerNameWithRealm].guildRank = guildRankName
    BPP_DB.PlayerInfoCache[playerNameWithRealm].rank = rank
...
function BPP_GetAndStorePlayerInfoFromUnit(unit, allowFriendly)
    ...
    class = BPP_ConvertClassToEnglish(class)
...
function BPP_GetRealmNameFromCharacterKey(characterKey)
...
local function GetPlayerInfoCacheKeyFromCharacterKeyAndEnemyPlayerName(characterKey, enemyPlayerName)
    ...
    if parsedRealm then
        ...
        local characterContextRealm = BPP_GetRealmNameFromCharacterKey(characterKey)
        if not characterContextRealm then
            characterContextRealm = BPP_RealmName -- Fallback
        end
...
function BPP_CleanupPlayerInfoCache()
    if not BPP_DB.PlayerKillCounts.Characters then return end

    local cleanedInfoCache = {}
    local playersToKeep = {}

    -- Collect names of all players who have been killed by us
    for characterKey, characterData in pairs(BPP_DB.PlayerKillCounts.Characters) do
    ...
    for characterKey, lossData in pairs(BPP_DB.PvPLossCounts) do
        if lossData.Deaths then
    ...
    for infoKey, data in pairs(BPP_DB.PlayerInfoCache) do
        if playersToKeep[infoKey] then
            cleanedInfoCache[infoKey] = data
        end
    end

    BPP_DB.PlayerInfoCache = cleanedInfoCache
end


function BPP_InitializeAchievementDataStructure()
    if not BPP_DB.CharacterAchievements then
        BPP_DB.CharacterAchievements = {}
    end

    if not BPP_DB.CharacterAchievementPoints then
        BPP_DB.CharacterAchievementPoints = {}
    end
    ...
    if not BPP_DB.CharacterAchievementPoints[characterKey] == nil then
        BPP_DB.CharacterAchievementPoints[characterKey] = 0
    end
end


function BPP_SaveAchievement(achievementID, completedDate, points)
    if not BPP_DB.CharacterAchievements then
    ...
    if not BPP_DB.CharacterAchievements[characterKey][achievementID] then
        BPP_DB.CharacterAchievements[characterKey][achievementID] = {}
    end

    BPP_DB.CharacterAchievements[characterKey][achievementID].unlocked = true
    BPP_DB.CharacterAchievements[characterKey][achievementID].completedDate = completedDate
    BPP_DB.CharacterAchievements[characterKey][achievementID].points = points or 0

    -- Recalculate total points
    BPP_UpdateTotalAchievementPoints()
end

-- Calculate total achievement points for the current character
function BPP_UpdateTotalAchievementPoints()
    local characterKey = BPP_GetCharacterKey()
    local totalPoints = 0

    if not BPP_DB.CharacterAchievements or not BPP_DB.CharacterAchievements[characterKey] then
        BPP_DB.CharacterAchievementPoints[characterKey] = 0
        return 0
    end
...
function BPP_LoadDefaultSettings()
    BPP_DB.EnableKillAnnounceMessages = true
    BPP_DB.IncludePlayerDetailsInAnnounce = false
    BPP_DB.IncludeGuildDetailsInAnnounce = false
    BPP_DB.EnableRecordAnnounceMessages = true
    BPP_DB.EnableMultiKillAnnounceMessages = true
    BPP_DB.MultiKillThreshold = 3
    BPP_DB.AnnounceChannel = "GROUP"

    BPP_DB.AutoBattlegroundMode = true
    BPP_DB.CountAssistsInBattlegrounds = true
    BPP_DB.ForceBattlegroundMode = false
    BPP_DB.CountKillsInBattlegrounds = true
    BPP_DB.CountDeathsInBattlegrounds = true

    BPP_DB.ShowKillMilestones = true
    BPP_DB.EnableKillMilestoneSound = true
    BPP_DB.ShowMilestoneForFirstKill = true
    BPP_DB.KillMilestoneInterval = 5
    BPP_DB.KillMilestoneAutoHideTime = 5
    BPP_DB.MilestoneFramePosition = {
        point="TOP",
        relativePoint="TOP",
        xOfs=0,
        yOfs=-100
    ...
    BPP_DB.EnableMultiKillSounds = true
    BPP_DB.SoundPack = "LoL"
    BPP_DB.EnableDeathSounds = false
    BPP_DB.EnableSingleKillSounds = false
    BPP_DB.ShowScoreInPlayerTooltip = true
    BPP_DB.ShowExtendedTooltipInfo = true
    BPP_DB.ShowAccountWideStats = false
    BPP_DB.ShowOwnAltsOnLeaderboard = true
    BPP_DB.CapAchievementProgress = false

    BPP_DB.KillAnnounceMessage = "Enemyplayername killed! x#"
    BPP_DB.KillStreakEndedMessage = "My kill streak of STREAKCOUNT has ended!"
    BPP_DB.NewKillStreakRecordMessage = "New personal best: Kill streak of STREAKCOUNT!"
    BPP_DB.NewMultiKillRecordMessage = "New personal best: MULTIKILLTEXT!"

    -- Kill Streak Popup Settings
    BPP_DB.AutoOpenKillStreakPopup = false
    BPP_DB.KillStreakPopupPosition = {
        point = "CENTER",
...
function BPP_InitializePlayerKillCounts()
    if not BPP_DB.PlayerKillCounts.Characters then
        BPP_DB.PlayerKillCounts.Characters = {}
    end

    -- Ensure all characters have required fields (sanity check for migration/updates)
    for key, data in pairs(BPP_DB.PlayerKillCounts.Characters) do
    ...
    if BPP_DB.PlayerKillCounts.Characters[characterKey].Level1KillTimestamps == nil then
        BPP_DB.PlayerKillCounts.Characters[characterKey].Level1KillTimestamps = {}
    end

    -- Initialize CurrentKillStreakPlayers if it doesn't exist (for existing saves)
    if BPP_DB.PlayerKillCounts.Characters[characterKey].CurrentKillStreakPlayers == nil then
        BPP_DB.PlayerKillCounts.Characters[characterKey].CurrentKillStreakPlayers = {}
    end

    -- Initialize new kill streak popup settings if they don't exist (backward compatibility)
    if BPP_DB.AutoOpenKillStreakPopup == nil then
        BPP_DB.AutoOpenKillStreakPopup = false
    end
    if BPP_DB.KillStreakPopupPosition == nil then
        BPP_DB.KillStreakPopupPosition = {
            point = "CENTER",
            relativePoint = "CENTER",
            xOfs = 0,
    ...
    if BPP_DB.IncludePlayerDetailsInAnnounce == nil then
        BPP_DB.IncludePlayerDetailsInAnnounce = false
    end
    if BPP_DB.IncludeGuildDetailsInAnnounce == nil then
        BPP_DB.IncludeGuildDetailsInAnnounce = false
    end
    if BPP_DB.ShowOwnAltsOnLeaderboard == nil then
        BPP_DB.ShowOwnAltsOnLeaderboard = true
    end
end

function BPP_InitializePlayerLossCounts()
    if not BPP_DB.PvPLossCounts then
        BPP_DB.PvPLossCounts = {}
    end

    local characterKey = BPP_GetCharacterKey()
    if not BPP_DB.PvPLossCounts[characterKey] then
        BPP_DB.PvPLossCounts[characterKey] = {
            Deaths = {}
        }
    end
end

function BPP_InitializeLeaderboardCache()
    if not BPP_DB.LeaderboardCache then
        BPP_DB.LeaderboardCache = {}
    end
end

function BPP_ResetAllStatsToDefault()
    BPP_DB.PlayerInfoCache = {}
    BPP_DB.PlayerKillCounts = {}
    BPP_DB.PvPLossCounts = {}
    BPP_DB.CharacterAchievements = {}
    BPP_DB.CharacterAchievementPoints = {}
    BPP_DB.LeaderboardCache = {}
