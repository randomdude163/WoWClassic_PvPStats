...
local function BPP_RunTaskQueue(taskQueue, onDone)
    local currentTask = 1
...
function BPP_StartIncrementalAchievementsCalculation()
    ...
    local taskQueue = {
        function()
            ...
            job._timeStatsTask = BPP_CreateIncrementalTimeBasedStatsTask(characterData, killLocationsPerSlice, function(result)
        ...
        function()
            BPP_GetStreakStats(true)
        end,
        TaskQueueDelayFrame(1),
        function()
            BPP_GetNameBasedStats(true)
        end,
        TaskQueueDelayFrame(1),
        function()
        ...
        TaskQueueDelayFrame(1),
        function()
            deathsByClassData = BPP_CalculateDeathsByClass(charactersToProcess)
        end,
        TaskQueueDelayFrame(1),
        function()
            local task = BPP_CreateIncrementalSummaryStatisticsTask(charactersToProcess, killLocationsPerSlice, function(result)
                summaryStats = result
    ...
    BPP_RunTaskQueue(taskQueue, function()
...
function BPP_SendAnnounceMessage(message)
...
function BPP_GetPetOwnerGUID(petGUID)
    if not petGUID or not BPP_IsPetGUID(petGUID) then return nil end

    if UnitExists("pet") and UnitGUID("pet") == petGUID then
        return BPP_PlayerGUID
    end
...
function BPP_GetNameFromGUID(guid)
    ...
    if guid == BPP_PlayerGUID then
        return BPP_CharacterName
    end
...
function BPP_Print(message)
    local BPP_CHAT_MESSAGE_R = 1.0
    local BPP_CHAT_MESSAGE_G = 1.0
    local BPP_CHAT_MESSAGE_B = 0.74
    DEFAULT_CHAT_FRAME:AddMessage(message, BPP_CHAT_MESSAGE_R, BPP_CHAT_MESSAGE_G, BPP_CHAT_MESSAGE_B)
end

function BPP_GetCharacterKey()
    return BPP_CharacterName .. "-" .. BPP_RealmName
end

function BPP_NormalizePlayerName(playerName)
    if not playerName or playerName == "" then
        return playerName
    end

    local _, infoKey = BPP_GetPlayerInfo(playerName)
    if infoKey then
        return infoKey
    end

    return BPP_GetInfoKeyFromName(playerName)
end

function BPP_IsSamePlayerName(candidateName, targetName)
    ...
    local candidateKey = BPP_GetInfoKeyFromName(candidateName)
    local targetKey = BPP_GetInfoKeyFromName(targetName)
...
function BPP_IsVersionAtLeast(versionStr, minVersion)
...
function BPP_GetMultiKillText(count)
...
function BPP_GetPlayerCoordinates()
...
function BPP_FormatLastKillTimespan(lastKillTimestamp, useServerTime)
...
function BPP_GetTimeAgo(timestamp, useServerTime)
    if not timestamp or timestamp <= 0 then
        return "Unknown"
    end

    local timeString = BPP_FormatLastKillTimespan(timestamp, useServerTime)
    if timeString then
...
function BPP_FormatKDRatio(totalKills, totalDeaths, kdRatio)
...
function BPP_CreateGoldHighlight(parent, height)
...
function BPP_TimestampToHour(timestamp, timezoneOffsetHours)
...
function BPP_IsTimestampInHourRange(timestamp, startHour, endHour, timezoneOffsetHours)
    local hour = BPP_TimestampToHour(timestamp, timezoneOffsetHours)
    if not hour then
        return false
    end
...
--[[
    ...
    1. Simple Time Range:
       ...
       In achievement: BPP_CountKillsByTimeRangeName("early_morning")

    2. Specific Weekday:
       Add to weekdayGroups: {"tuesday_only", {WEEKDAY.TUESDAY}}
       In achievement: BPP_CountKillsByWeekdayGroup("tuesday_only")

    3. Weekend Nights:
       Add to combinations: {22, 6, {WEEKDAY.SATURDAY, WEEKDAY.SUNDAY}, "weekend_nights"}
       In achievement: BPP_CountKillsByCombination("weekend_nights")
    ...
    4. Complex Condition:
       ...
       In achievement: BPP_CountKillsBySpecialCondition("summer_evenings")
--]]
...
function BPP_GetLocalTimezoneOffset()
...
function BPP_CalculateAllTimeBasedStats()
    local BPP_DB = BPP_DB
    ...
    local characterData = BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters and BPP_DB.PlayerKillCounts.Characters[characterKey]
...
function BPP_CreateIncrementalTimeBasedStatsTask(characterData, maxKillLocationsPerFrame, onComplete)
        ...
        while processed < sliceBudget do
            ...
            if not locations or #locations == 0 then
                ...
                if locationIndex > #locations then
                    ...
                    if timestamp then
                        -- Keep the same per-kill logic as BPP_CalculateAllTimeBasedStats(),
...
function BPP_GetTimeBasedStats()
    if not TimeStatsCache then
        TimeStatsCache = BPP_CalculateAllTimeBasedStats()
    end

    return TimeStatsCache
end

function BPP_CountKillsInTimeRange(startHour, endHour, timezoneOffsetHours)
    local stats = BPP_GetTimeBasedStats()
    local key = startHour .. "_" .. endHour
    return stats.timeRanges[key] or 0
end

function BPP_CountKillsOnWeekdays(weekdays, timezoneOffsetHours)
    local stats = BPP_GetTimeBasedStats()
    local count = 0
...
function BPP_CountKillsInTimeRangeOnWeekdays(startHour, endHour, weekdays, timezoneOffsetHours)
    local stats = BPP_GetTimeBasedStats()
        ...
        if playerData.killLocations then
            for _, killLocation in ipairs(playerData.killLocations) do
                if killLocation.timestamp and
                   BPP_IsTimestampInHourRange(killLocation.timestamp, startHour, endHour, timezoneOffsetHours) and
                   BPP_IsTimestampOnWeekday(killLocation.timestamp, weekdays, timezoneOffsetHours) then
                    count = count + 1
                end
            end
        end
...
function BPP_TestTimeZoneOffsetCalculation()
    ...
    local localOffset = BPP_GetLocalTimezoneOffset()
    BPP_Print("SUCCESS: Found " .. totalKills .. " total kills")
    BPP_Print("Local timezone offset: UTC" .. (localOffset >= 0 and "+" or "") .. localOffset)

    -- Debug timezone calculation
    local currentTime = time()
    local utc_date = date("!*t", currentTime)
    local local_date = date("*t", currentTime)
    BPP_Print("DEBUG: UTC time: " .. utc_date.hour .. ":" .. string.format("%02d", utc_date.min))
    BPP_Print("DEBUG: System local time: " .. local_date.hour .. ":" .. string.format("%02d", local_date.min))
    BPP_Print("DEBUG: Raw hour difference: " .. (local_date.hour - utc_date.hour))

    -- Test work hours calculation
    local stats = BPP_GetTimeBasedStats()
    local workHoursKills = stats.combinations.workhours_weekdays or 0
    BPP_Print("Work hours weekdays kills: " .. workHoursKills)

    -- Test current time WITHOUT manual timezone adjustment (date("*t") already handles local time)
    local dateInfo = date("*t", currentTime)
    BPP_Print("Current calculated local time: " .. dateInfo.hour .. ":" .. string.format("%02d", dateInfo.min))
    BPP_Print("Current weekday: " .. dateInfo.wday .. " (1=Sunday, 2=Monday, etc.)")
    BPP_Print("Is work hours? " .. (dateInfo.hour >= 9 and dateInfo.hour < 17 and dateInfo.wday >= 2 and dateInfo.wday <= 6 and "YES" or "NO"))

    BPP_Print("=== TIMEZONE FIX APPLIED ===")
    BPP_Print("Fixed: All time functions now use date('*t') without manual timezone offset")
    BPP_Print("This prevents double-applying timezone conversion")
    BPP_Print("=== End Test ===")
end

function BPP_IsTimestampOnWeekday(timestamp, weekdays, timezoneOffsetHours)
...
function BPP_IsTimestampOnDate(timestamp, day, month, timezoneOffsetHours)
...
function BPP_CountKillsOnDate(day, month, timezoneOffsetHours)
    local count = 0
    local characterKey = BPP_GetCharacterKey()
    local characterData = BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters and BPP_DB.PlayerKillCounts.Characters[characterKey]
    ...
    for playerKey, playerData in pairs(characterData.Kills) do
        if playerData.killLocations then
            for _, killLocation in ipairs(playerData.killLocations) do
                if killLocation.timestamp and BPP_IsTimestampOnDate(killLocation.timestamp, day, month, timezoneOffsetHours) then
                    count = count + 1
                end
            end
        end
...
function BPP_IsTimestampOnFridayThe13th(timestamp, timezoneOffsetHours)
    if not timestamp then
        return false
    end
...
function BPP_CountKillsOnFridayThe13th(timezoneOffsetHours)
    local count = 0
    local characterKey = BPP_GetCharacterKey()
    local characterData = BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters and BPP_DB.PlayerKillCounts.Characters[characterKey]
    ...
    for playerKey, playerData in pairs(characterData.Kills) do
        if playerData.killLocations then
            for _, killLocation in ipairs(playerData.killLocations) do
                if killLocation.timestamp and BPP_IsTimestampOnFridayThe13th(killLocation.timestamp, timezoneOffsetHours) then
                    count = count + 1
                end
            end
        end
...
function BPP_IsTimestampInMonth(timestamp, month, timezoneOffsetHours)
...
function BPP_CountKillsInMonth(month, timezoneOffsetHours)
    local count = 0
    local characterKey = BPP_GetCharacterKey()
    local characterData = BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters and BPP_DB.PlayerKillCounts.Characters[characterKey]
    ...
    for playerKey, playerData in pairs(characterData.Kills) do
        if playerData.killLocations then
            for _, killLocation in ipairs(playerData.killLocations) do
                if killLocation.timestamp and BPP_IsTimestampInMonth(killLocation.timestamp, month, timezoneOffsetHours) then
                    count = count + 1
                end
            end
        end
...
function BPP_CalculateAllNameBasedStats()
    local characterKey = BPP_GetCharacterKey()
    local characterData = BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters and BPP_DB.PlayerKillCounts.Characters[characterKey]
...
function BPP_GetNameBasedStats(forceRefresh)
    if not nameStatsCache or forceRefresh then
        nameStatsCache = BPP_CalculateAllNameBasedStats()
    end
    return nameStatsCache
end

function BPP_CountKillsStartingWithLetter(letter)
    local stats = BPP_GetNameBasedStats()
    letter = string.upper(letter)
    return stats.byFirstLetter[letter] or 0
end

function BPP_CountKillsByLength(length)
    local stats = BPP_GetNameBasedStats()
    return stats.byNameLength[length] or 0
end

function BPP_CountKillsWithNameStartingWith(letter)
    local stats = BPP_GetNameBasedStats()
    letter = string.upper(letter)
    return stats.byFirstLetter[letter] or 0
end

function BPP_CountKillsWithNameLength(length)
    local stats = BPP_GetNameBasedStats()
    return stats.byNameLength[length] or 0
end

function BPP_CountKillsWithSpecialCharsInName()
    local stats = BPP_GetNameBasedStats()
    return stats.withSpecialChars or 0
end

function BPP_CountKillsWithAccentCharsInName()
    local stats = BPP_GetNameBasedStats()
    return stats.withAccentChars or 0
end

function BPP_GetCurrentAchievementPoints()
    local characterKey = BPP_GetCharacterKey()
    return BPP_DB.CharacterAchievementPoints and BPP_DB.CharacterAchievementPoints[characterKey] or 0
end

function BPP_GetUnlockedAchievementCount()
    local characterKey = BPP_GetCharacterKey()
    if not BPP_DB or not BPP_DB.CharacterAchievements or not BPP_DB.CharacterAchievements[characterKey] then
        return 0
    end

    local count = 0
    for id, data in pairs(BPP_DB.CharacterAchievements[characterKey]) do
...
function BPP_CalculateAchievementCompletion(achievement, stats)
...
function BPP_CalculateAllStreakStats()
    local BPP_DB = BPP_DB
    local BPP_GetCharacterKey = BPP_GetCharacterKey
    local pairs = pairs
    local ipairs = ipairs
...
function BPP_GetStreakStats(forceRefresh)
    if not streakStatsCache or forceRefresh then
        streakStatsCache = BPP_CalculateAllStreakStats()
    end
...
function BPP_CountConsecutiveDaysWithMinKills(minKills)
    local streakStats = BPP_GetStreakStats()
    return streakStats[minKills] or 0
end

-- Function to count total days with at least minKills (non-consecutive)
function BPP_CountTotalDaysWithMinKills(minKills)
...
function BPP_CalculateTimePeriodBoundaries()
    local currentTime = time()
...
function BPP_ClearGUIDCache()
    GUIDToNPCIDCache = {}
end

function BPP_GetNPCIDFromGUID(guid)
    if not guid then return nil end
...
function BPP_ShowWhatsNewPopup(titleText, messageText, onCloseCallback, linkUrl)
    local frame = CreateFrame("Frame", "BPP_WhatsNewPopup", UIParent, BackdropTemplateMixin and "BackdropTemplate")
