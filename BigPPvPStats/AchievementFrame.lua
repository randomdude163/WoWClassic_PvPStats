...
local AchievementFrame = CreateFrame("Frame", "BPPAchievementFrame", UIParent, "BasicFrameTemplateWithInset")
...
if AchievementFrame.CloseButton then
    AchievementFrame.CloseButton:SetScript("OnClick", function()
        BPP_FrameManager:HideFrame("Achievements")
    end)
end

tinsert(UISpecialFrames, "BPPAchievementFrame")
...
local scrollFrame = CreateFrame("ScrollFrame", "BPPAchievementScrollFrame", contentFrame, "UIPanelScrollFrameTemplate")
...
local scrollContent = CreateFrame("Frame", "BPPAchievementContent", scrollFrame)
    ...
    for _, achievement in ipairs(achievements) do
        ...
        if prefix == category:lower() or (category:lower() == "cities" and prefix == "city") then
            if prefix == "class" then
                ...
                elseif BPP_GameVersion == BPP_GAME_VERSIONS.CLASSIC then
    ...
    for nameWithLevel, killData in pairs(charData.Kills or {}) do
        ...
        if enemyLevel >= 1 and killData.killLocations then
            for _, location in ipairs(killData.killLocations) do
                ...
                if playerLevel then
                    local grayThreshold = BPP_GrayLevelThreshods[playerLevel]
...
function BPP_CalculateGrayKills()
    local currentCharacterKey = BPP_GetCharacterKey()
    local charData = BPP_DB.PlayerKillCounts.Characters[currentCharacterKey]
...
local function CreatePointsImage(tile, pointsValue)
    local function GetPointsImagePath(points)
        local basePath = "Interface\\AddOns\\BigPPvPStats\\achievement_img\\Achievement_icon"
        if points == 10 then
            return basePath .. "10"
...
local function CreateTitleAndDescription(tile, icon, pointsImage, achievement)
    ...
    title:SetText(BPP_ReplacePlayerNamePlaceholder(titleText, nil, achievement))
...
local function CreateProgressBar(tile, targetValue, currentProgress, achievement, icon, title)
    ...
    if achievement.unlocked then
        local displayProgress = BPP_DB.CapAchievementProgress and targetValue or currentProgress
...
local function CreateAchievementTile(i, achievement, savedAchievements)
    ...
    tile:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine(BPP_ReplacePlayerNamePlaceholder(titleText, nil, achievement), r, g, b) -- Rarity-colored title
        ...
        local personalizedSubText = BPP_ReplacePlayerNamePlaceholder(subTextValue, nil, achievement)
    ...
    tile:SetScript("OnClick", function(self, button)
        if button == "LeftButton" and IsControlKeyDown() and achievement.unlocked then
            BPP_ShareAchievementInChat(achievement)
...
local function LoadAchievementCompletionStatus()
    ...
    for _, achievement in ipairs(achievements) do
        ...
        local savedData = BPP_DB.CharacterAchievements[characterKey][achievement.id]
...
function BPP_ToggleAchievementFrame()
    if AchievementFrame:IsShown() then
        ...
        if not BPP_FrameManager.frames["Achievements"] then
            BPP_FrameManager:RegisterFrame(AchievementFrame, "Achievements")
        end
        BPP_FrameManager:ShowFrame("Achievements")
    end
end
