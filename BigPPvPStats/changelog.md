v1.0.1:
- Fixed a serious bug: the Nearby panel was registered with the frame manager as a keyboard-capturing "interactive" window (missing a flag that every other passive popup already had set correctly). Since it auto-shows/re-shows itself on nearly every detection, it was almost always the most-recently-shown frame and would grab ALL keyboard input into itself, locking the player out of moving or typing in-game until they closed and reopened WoW. It's now correctly registered as passive, like the KOS/Stealth/Trash popups always were
- Fixed the Nearby panel and Kill On Sight not detecting anyone from nameplates alone until after the first manual mouseover/target: the addon's own player-info lookup indexed `UnitClass()`'s return value directly, which errors if that unit's class hasn't been resolved client-side yet (routine for a nameplate that's never been moused over) - silently aborting the whole detection with no addon error message, only a debug-mode print. Class/race now fall back to "Unknown" like a guildless player's guild info already did, instead of erroring out
- Removed the "What's new" popup that appeared on login after every version bump
- Fixed the Settings window growing too tall for the screen (930px, plus tabs hanging further below that) after several rounds of new sections being added to Kill On Sight - each tab's content now scrolls internally within a fixed, screen-friendly window size (620px) instead of the window itself growing every time something's added
