BPP_KillsListFrame = nil

BPP_SortKillsListBy = "lastKill"
BPP_SortKillsListAscending = false
local KILLS_FRAME_WIDTH = 1135
local KILLS_FRAME_HEIGHT = 550

BPP_KillsListFrameInitialSetup = true
...
local function CreateColumnHeader(parent, text, width, anchor, xOffset, yOffset, columnId)
    ...
    button:SetScript("OnClick", function()
        if BPP_SortKillsListBy == columnId then
            BPP_SortKillsListAscending = not BPP_SortKillsListAscending
        else
            BPP_SortKillsListBy = columnId
            BPP_SortKillsListAscending = false
        end
    ...
    if BPP_SortKillsListBy == columnId then
        local sortIndicator = BPP_SortKillsListAscending and " ^" or " v"
...
function BPP_GetCharactersToProcessForStatistics()
...
function BPP_IsPlayerFromDifferentRealm(playerName)
    ...
    return false, playerName, BPP_RealmName
end
...
local function FormatPlayerNameForDisplay(playerName)
    local isFromDifferentRealm, cleanName, realm = BPP_IsPlayerFromDifferentRealm(playerName)
...
local function CreateLastKillCell(content, anchorTo, lastKill, width)
    ...
    if lastKill and lastKill > 0 then
        local timeInfo = BPP_GetTimeAgo(lastKill)
...
local function CreateEntryRow(content, entry, yOffset, colWidths, isAlternate)
    ...
    rowContainer:SetScript("OnClick", function(self, button)
        if button == "LeftButton" then
            BPP_ShowPlayerDetailFrame(entry.name)
        end
    end)
    ...
    rowContainer:SetScript("OnMouseDown", function(self, button)
        if button == "RightButton" then
            if not BPP_PlayerRowDropDown then
                CreateFrame("Frame", "BPP_PlayerRowDropDown", UIParent, "UIDropDownMenuTemplate")
            end

            UIDropDownMenu_Initialize(BPP_PlayerRowDropDown, function(self, level)
                local info = UIDropDownMenu_CreateInfo()
                info.text = "Copy Name: " .. entry.name
                info.notCheckable = true
                info.func = function()
                    if BPP_CopyBox then
                        if BPP_CopyContainer then
                            BPP_CopyContainer:Hide()
                        end
                        BPP_CopyBox:Hide()
                        BPP_CopyBox = nil
                        BPP_CopyContainer = nil
                    end

                    local copyContainer = CreateFrame("Frame", "BPP_CopyContainer", UIParent)
                    ...
                    local copyBox = CreateFrame("EditBox", "BPP_CopyBox", copyContainer, "InputBoxTemplate")
                    ...
                    BPP_CopyBox = copyBox
                    BPP_CopyContainer = copyContainer
                end
                ...
                infoNote.func = function()
                    BPP_ShowPlayerDetailFrame(entry.name, true)
                end
                UIDropDownMenu_AddButton(infoNote)
            end, "MENU")

            ToggleDropDownMenu(1, nil, BPP_PlayerRowDropDown, self, 0, 0)
            return
        end
    end)
...
local function CreateMainFrame()
    local frame = CreateFrame("Frame", "BPP_KillStatsFrame", UIParent, "BasicFrameTemplateWithInset")
    ...
    table.insert(UISpecialFrames, "BPP_KillStatsFrame")
    local titleText = BPP_GetFrameTitleTextWithCharacterText("PvP History")
    frame.TitleText:SetText(titleText)

    -- Override close button to work in combat
    if frame.CloseButton then
        frame.CloseButton:SetScript("OnClick", function()
            BPP_FrameManager:HideFrame("KillsList")
        end)
    end
...
function BPP_RefreshKillsListFrame()
    if BPP_KillsListFrameInitialSetup then
        return
    end

    if BPP_KillsListFrame == nil then
        return
    end

    local content = BPP_KillsListFrame.content
    if not content then
        return
    end

    local titleText = BPP_GetFrameTitleTextWithCharacterText("PvP History")
    BPP_KillsListFrame.TitleText:SetText(titleText)

    CleanupFrameElements(content)

    local yOffset = CreateColumnHeaders(content)
    local sortedEntries = BPP_FilterAndSortEntries()
...
function BPP_CreateKillsListFrame()
    if (BPP_KillsListFrame) then
        BPP_FrameManager:ShowFrame("KillsList")
        BPP_RefreshKillsListFrame()
        return
    end

    BPP_KillsListFrame = CreateMainFrame()
    BPP_KillsListFrame.content = CreateScrollFrame(BPP_KillsListFrame)
    BPP_CreateSearchBar(BPP_KillsListFrame)

    BPP_FrameManager:RegisterFrame(BPP_KillsListFrame, "KillsList")

    local titleText = BPP_GetFrameTitleTextWithCharacterText("Player Kills List")
    BPP_KillsListFrame.TitleText:SetText(titleText)

    for i = #UISpecialFrames, 1, -1 do
        if (UISpecialFrames[i] == "BPP_KillStatsFrame") then
    ...
    C_Timer.After(0.01, function()
        BPP_KillsListFrameInitialSetup = false
        BPP_RefreshKillsListFrame()
    end)
end
