...
local function CreateAchievementPopupFrame()
    local frame = CreateFrame("Frame", "BigPPvPStatsAchievementPopup", UIParent, BackdropTemplateMixin and "BackdropTemplate")
    ...
    if BPP_FrameManager then
        BPP_FrameManager:RegisterFrame(frame, "AchievementPopup", true)
    end
...
function AchievementPopup:DisplayPopup(achievementData)
    ...
    if BPP_FrameManager then
        BPP_FrameManager:BringToFront("AchievementPopup")
    end
