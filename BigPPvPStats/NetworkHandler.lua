...
function Network:ApplyPlayerProfile(summaryTable, nameField, raceField, genderField, classField, charactersToProcess)
    ...
    if not HasKnownProfile(playerInfo) and string.find(playerName, "%-") then
        ...
        if shortName and shortName ~= "" then
            local shortInfo = BPP_GetPlayerInfo(shortName)
        ...
        for _, characterData in pairs(charactersToProcess) do
            if characterData and characterData.Kills then
                for nameWithLevel, killData in pairs(characterData.Kills) do
                    ...
                    if BPP_IsSamePlayerName(killName, playerName) then
                        playerInfo = {
                            race = killData.race,
...
local function ParseCharacterKey(characterKey)
    ...
    return characterKey, BPP_RealmName
end
...
local function GetCharacterLastActivityTimestamp(characterKey, characterData)
    ...
    if BPP_DB and BPP_DB.PvPLossCounts and BPP_DB.PvPLossCounts[characterKey] and BPP_DB.PvPLossCounts[characterKey].Deaths then
        for _, deathEntry in pairs(BPP_DB.PvPLossCounts[characterKey].Deaths) do
...
local function GetCharacterAchievementStats(characterKey)
    ...
    if BPP_DB and BPP_DB.CharacterAchievements and BPP_DB.CharacterAchievements[characterKey] then
        for _, achievementData in pairs(BPP_DB.CharacterAchievements[characterKey]) do
    ...
    if BPP_DB and BPP_DB.CharacterAchievementPoints and BPP_DB.CharacterAchievementPoints[characterKey] ~= nil then
        points = tonumber(BPP_DB.CharacterAchievementPoints[characterKey]) or points
    end
...
function Network:BuildDetailedStatsForCharacter(characterKey, characterData)
    ...
    local infoKey = BPP_GetPlayerInfoKey(charName, charRealm)
    local charInfo = BPP_DB and BPP_DB.PlayerInfoCache and BPP_DB.PlayerInfoCache[infoKey]
    ...
    result.addonVersion = BPP_GetAddonVersion()
...
function Network:BuildDetailedStats()
    ...
    local charactersToProcess = {}
    local currentCharacterKey = BPP_GetCharacterKey()
    if BPP_DB and BPP_DB.PlayerKillCounts and BPP_DB.PlayerKillCounts.Characters then
...
function Network:ConstructPayload(components)
    ...
    return {
        ...
        class = BPP_ConvertClassToEnglish(UnitClass("player")),
        race = BPP_ConvertRaceToEnglish(UnitRace("player")),
        faction = UnitFactionGroup("player") or "",
        timestamp = GetServerTime(),
        realm = BPP_RealmName,
        -- Optimization: Send version without "v" prefix
        addonVersion = BPP_GetAddonVersion(),
        achievementsUnlocked = BPP_GetUnlockedAchievementCount(),
        totalAchievements = totalAchievements,
        achievementPoints = BPP_GetCurrentAchievementPoints()
        }
end
...
function Network:BroadcastStats(providedStats)
    ...
    if BPP_RefreshLeaderboardFrame and BPP_LeaderboardFrame and BPP_LeaderboardFrame:IsShown() then
...
function Network:ValidateLeaderboardCache()
    if not BPP_DB.LeaderboardCache then return end

    local fixedCount = 0
    for playerName, entry in pairs(BPP_DB.LeaderboardCache) do
        local changed = false
    ...
    if fixedCount > 0 and BPP_Debug then
...
function Network:OnCommReceived(prefix, message, distribution, sender)
    ...
    if not senderRealm then
        senderName = sender
        senderRealm = BPP_RealmName -- Default to our realm
    end

    -- Ignore messages from ourselves (even if sent across channels like YELL/GUILD)
    if senderName == UnitName("player") and senderRealm == BPP_RealmName then return end
    ...
    if msgType == "FULL" then
        ...
        if statsData.addonVersion and BPP_IsAddonVersionNewer(statsData.addonVersion) then
            BPP_ShowAddonUpdatePopup(statsData.addonVersion)
        end
        ...
        if statsData.realm and statsData.realm ~= BPP_RealmName then
        ...
        if BPP_LeaderboardFrame and BPP_LeaderboardFrame:IsShown() then
            BPP_RefreshLeaderboardFrame()
        end
...
function Network:GetAllLeaderboardData(includeOwnAlts)
    ...
    if BPP_DB.LeaderboardCache then
        for name, data in pairs(BPP_DB.LeaderboardCache) do
